...
@pytest.mark.parametrize("draft_path", [None, "/tmp/qwen35/mtp.engine"])
def test_speculative_decoding_config_pickle(draft_path):
    config = trtllm.SpeculativeDecodingConfig(True)
    config.mtp_draft_engine_path = draft_path
    config_copy = pickle.loads(pickle.dumps(config))
    assert config_copy.fast_logits == True
    assert config_copy.mtp_draft_engine_path == draft_path
