    ...
    for (auto const& requests : {contextRequests, genRequests})
    {
        for (auto const& llmReq : requests)
        {
            ...
            auto const maxBeamBlockCount
                = kvCacheManager->copyAttentionBlockOffsets(*kvCacheBlockOffsetsHost, numSequences, requestId);
            maxBlockCount = std::max(maxBlockCount, maxBeamBlockCount);
            if (crossKvCacheBlockOffsetsHost)
            {
                auto const maxCrossBeamBlockCount = crossKvCacheManager->copyAttentionBlockOffsets(
                    *crossKvCacheBlockOffsetsHost, numSequences, requestId);
