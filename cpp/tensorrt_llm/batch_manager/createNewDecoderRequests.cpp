...
void CreateNewDecoderRequests::refreshExternalDraftTokens(RequestVector const& requests,
    runtime::decoder::DecoderState& decoderState, runtime::ModelConfig const& modelConfig,
    runtime::WorldConfig const& worldConfig, runtime::CudaStream const& decoderStream) const
{
    for (auto const& request : requests)
    {
        auto const slot = request->mSeqSlot.value();
        auto const count = request->getNumDraftTokens() + 1;
        decoderState.setNumDecodingEngineTokens(slot, count);
        newRequestDraftTokensExternal(decoderState.getJointDecodingInput(), slot, *request, count, modelConfig,
            worldConfig, mSpeculativeDecodingFastLogits, mIsLeaderInOrchMode, decoderStream);
    }
}
